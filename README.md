...
transcription evidence. It also contains the repeatable protocol exercises:
generated synthetic PCM WAV sent to `/v1/audio/analyze`, ordered PCM chunks
sent over `/v1/audio/stream/{session}`, and synthetic tool-policy calls through
`/v1/demo/tools/{tool}`. The tool inspector uses the same fixed definitions as
the stdio MCP server; it never connects to an external account or accepts
caller-provided identity scopes.
...
![FinVoice AI Model Lab workflow](docs/lab.gif)

Model Lab is deliberately separate from the support-console experience. It is
for local research and contract inspection, not customer use: upload a
consented PCM WAV fixture or record locally, inspect VAD and transcription
evidence, then run isolated synthetic speech, streaming, and tool-policy
exercises. On load, the lab queries `/v1/demo/runtime` and labels the active
output mode so a repeatable fixture cannot be mistaken for model inference.

```text
[WAV upload / local microphone] --> [browser PCM WAV conversion]
                                             |
                                             v
                                  POST /v1/audio/analyze
                                             |
                    VAD segments + transcript + confidence + active model label
                                             |
                  deterministic fixture OR local Faster Whisper inference
```

The default `deterministic-transcription-v1` path deliberately returns a
repeatable contract fixture; it does not establish transcription quality or
semantic understanding. The generated sine-wave exercise similarly validates
transport and VAD behavior, but is not speech. With the optional ASR profile,
Faster Whisper runs locally against actual audio, but results still depend on
the selected model/version, decoding configuration, audio quality, language,
and hardware. The lab neither persists recordings nor provides consent,
retention, access-control, benchmark-governance, or real-user identity
controls.

A production implementation would require explicit recording consent and data
minimization, encrypted transport and storage with retention/deletion policy,
verified identity and access control, a governed and representative evaluation
dataset, versioned ASR/model releases with monitoring and rollback, redacted
telemetry, and a human escalation path. None of those production controls are
provided by Model Lab.

### From deterministic contracts to local model testing

The default path is designed to make service behavior repeatable; it is not
evidence that generated language or synthetic audio has been understood. The
deterministic provider makes policy, retrieval, citation, streaming-state, and
failure-path tests stable. Its fixed transcription and template response should
therefore be treated as a contract fixture, not a model-quality result.

```text
                         REPEATABLE CONTRACT PATH
[versioned requests / synthetic PCM] --> [deterministic ASR + template generator]
                                             |
                     policy / retrieval / citations / endpointing / escalation
                                             |
                                             v
                              stable expected events and API responses

                         LOCAL MODEL TEST PATH
[consented microphone recording or versioned WAV fixture]
                         |
                         v
...
The showcase's generated sine-wave sample remains useful for exercising WAV,
VAD, and transport contracts, but it is not speech and may produce an empty
Faster Whisper transcript. A thorough local evaluation should use only
consented recordings or clearly labeled synthetic speech fixtures, and should
cover the following layers:

| Layer | Local test method | Evidence to retain |
| --- | --- | --- |
| Audio ingress | Valid/invalid PCM WAV, duration/size limits, silence, noise, and microphone recordings | Validation outcomes and VAD segments |
| ASR | Fixed, consented WAV fixtures across languages, accents, noise, and code-switching | WER/CER, transcript, model/version, decoding settings, and latency |
| Streaming | Ordered chunks, duplicate/out-of-order chunks, silence endpointing, and interruption | Event trace: accepted, speech-started, partial, finalized, or error |
